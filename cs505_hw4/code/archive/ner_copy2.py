...
def conputePar(sents, vocab, tag):
    ...
    for list in sents:
        ...
        for i in range(seq_len - 1):
            current_tagid = tag[list[i][-1]]
            next_tagid = tag[list[i + 1][-1]]
...
if __name__ == "__main__":
    ...
    for sent in test_sents:
        ...
        for i in range(len(sent)):
            if sent[i][0] not in train_vocab:
                ...
                X_dev.append(train_vocab["UNK"]-1)
            else:
    ...
    y_pred_id = model.predict(X_dev,test_lengths)
    print(y_pred_id)
