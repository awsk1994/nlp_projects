...
def shared(shape, name):
    ...
    return T.shared(value=value.astype(T.config.floatX), name=name)
...
def evaluate(parameters, f_eval, raw_sentences, parsed_sentences,
    ...
    eval_id = int(time.time()*1000.0) # ms # np.random.randint(1000000, 2000000)
    ...
    print("count trace = {}".format(count.trace()))
    print("count.sum() = {}".format(count.sum()))
    print("trace = {}".format((100. * count.trace() / max(1, count.sum()))))
...
def write_output_prediction(parameters, f_eval, raw_sentences, parsed_sentences,
             id_to_tag, dictionary_tags):
    """
    Evaluate current model using CoNLL script.
    """
